            ...
            try (Block<ByteBuffer> block = new Block<ByteBuffer>(blockByteBuffer, byteCompare, MemoryManagers.heap(),
                ...
                try (BlockIterator<ByteBuffer> blockIterator = block.iterator()) {
                    ...
                    if (reverseEntries.size() > 0) {
                        ...
                        blockIterator.seekToLast();
                        BlockHelper.assertReverseSequence(blockIterator,
                    ...
                    for (BlockEntry<ByteBuffer> entry : entries) {
                        List<BlockEntry<ByteBuffer>> nextEntries = entries.subList(entries.indexOf(entry),
                                entries.size());
                        List<BlockEntry<ByteBuffer>> prevEntries = reverseEntries.subList(
                                reverseEntries.indexOf(entry), reverseEntries.size());
