...
public final class TableIterator extends AbstractReverseSeekingIterator<Slice, Slice>
{
    ...
    protected enum CurrentOrigin{
       /*
        * reversable iterators don't have a consistent concept of a "current" item
        * instead they exist in a position "between" next and prev.
        * in order to make the BlockIterator 'current' work, we need to track from which direction
        * it was initialized so that calls to advance the encompassing 'blockIterator' are consistent
        */
       PREV, NEXT, NONE
