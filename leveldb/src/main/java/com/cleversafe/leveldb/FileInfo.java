...
public class FileInfo {
  public enum FileType {
    LOG, DB_LOCK, TABLE, MANIFEST, CURRENT, TEMP,
    /**
  ...
  // TODO maybe identify level at creation time
  private final FileInfo.FileType fileType;
  private final long fileNumber;
  private final DBHandle db;

  protected FileInfo(final DBHandle db, final FileInfo.FileType fileType) {
    this(db, fileType, -1);
  }

  protected FileInfo(final DBHandle db, final FileInfo.FileType fileType, final long fileNumber) {
    this.fileType = fileType;
    this.fileNumber = fileNumber;
    this.db = db;
  }

  public static FileInfo log(final DBHandle db, final long number) {
    return new FileInfo(db, FileType.LOG, number);
  }

  public static FileInfo table(final DBHandle db, final long number) {
    return new FileInfo(db, FileType.TABLE, number);
  }

  public static FileInfo lock(final DBHandle db) {
    return new FileInfo(db, FileType.DB_LOCK);
  }

  public static FileInfo current(final DBHandle db) {
    return new FileInfo(db, FileType.CURRENT);
  }

  public static FileInfo temp(final DBHandle db, final long number) {
    return new FileInfo(db, FileType.TEMP, number);
  }

  public static FileInfo manifest(final DBHandle db, final long number) {
    return new FileInfo(db, FileType.MANIFEST, number);
  }

  public static FileInfo legacyInfoLog(final DBHandle db) {
